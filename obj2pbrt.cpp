...
int obj2pbrt(std::vector<std::string> args, osg::Vec3 eye, osg::Vec3 lookAtTarget, osg::Vec3 upDir, double fov, double scale, osg::Vec3 _)
{
    std::string objFilename, pbrtFilename;
    ...
    std::vector<material_t> materials;
    std::string err;
    int pos1 = objFilename.find_last_of('\\');
    int pos2 = objFilename.find_last_of('/');
    int pos = std::max(pos1, pos2);
    std::string mtl_basepath = objFilename.substr(0, pos + 1);
    if (!LoadObj(shapes, materials, err, objFilename.c_str(), // /* mtl_basepath */ nullptr,
                 mtl_basepath.c_str(),
                 ptexQuads ? 0 : load_flags_t(triangulation)))
    {
        fprintf(stderr, "%s: errors loading OBJ file: %s\n", objFilename.c_str(), err.c_str());
        return 1;
    }

    FILE *f = fopen(pbrtFilename.c_str(), "w");
    if (!f)
    {
        perror(pbrtFilename.c_str());
        return 1;
    }
    ...
    std::ifstream ifs("pbrt/scene.template.pbrt");
    std::stringstream ss;
    ss << ifs.rdbuf();
    std::string sceneTemplate = ss.str();
    std::string insertSlot = "${ObjData}";
    int insertPos = sceneTemplate.find(insertSlot);
    std::string sceneTemplatePart1 = sceneTemplate.substr(0, insertPos);
    std::string sceneTemplatePart2 = sceneTemplate.substr(insertPos + insertSlot.size());

    // 图片左右翻转
    fprintf(f, "Scale -1 1 1\n");
    // LookAt
    fprintf(f, "LookAt %f %f %f  %f %f %f  %f %f %f\n",
            eye.x(), eye.y(), eye.z(),
            lookAtTarget.x(), lookAtTarget.y(), lookAtTarget.z(),
            upDir.x(), upDir.y(), upDir.z());
    // Camera
    fprintf(f, "Camera \"perspective\" \"float fov\" [ %f ]\n", fov);
    // part 1
    fprintf(f, sceneTemplatePart1.c_str());

    double x1 = bounds[0][0];
    double y1 = bounds[0][1];
    double z1 = bounds[0][2];
    double x2 = bounds[1][0];
    double y2 = bounds[1][1];
    double z2 = bounds[1][2];
...
//    // 计算缩放和平移
//    double maxEdge = std::max(std::max(sizeX, sizeY), sizeZ);
//    double scale = 50.0 / maxEdge;

    double tx = -centerX * scale;
    double ty = -centerY * scale;
    double tz = -centerZ * scale;

    osg::Matrix m1;
    m1.makeTranslate(-centerX, -centerY, -centerZ);
    osg::Matrix m2;
    m2.makeScale(scale, scale, scale);
    osg::Matrix m3;
    m3.makeRotate(osg::PI_2, osg::Vec3(1, 0, 0));
    ...
    for (const shape_t &shape : shapes)
    {
        ...
        // 对于pbrt而言，变换是按照顺序叠加的，而非一次性的
        // 故而顺序是非常重要的
        // fprintf(f, "Scale %f %f %f\n", scale, scale, scale);
        // fprintf(f, "Translate %f %f %f\n", -centerX, -centerY, -centerZ);
        // fprintf(f, "Rotate 90 1 0 0\n");
        fprintf(f, "Transform [ %f %f %f %f\n    %f %f %f %f\n    %f %f %f %f\n    %f %f %f %f ]\n",
...
//                matrix(0, 3), matrix(1, 3), matrix(2, 3), matrix(3, 3));
        ...
        for (int id : materialIds)
        {
            ...
            else
            {
                ...
                for (iter = mtl.unknown_parameter.begin(); iter != mtl.unknown_parameter.end(); ++iter)
...
//                        (p.z - center.z()) * scale);
    ...
    fprintf(f, sceneTemplatePart2.c_str());
